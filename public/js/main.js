...
$(document).ready(function(){
    ...
    $('a.notes').click(function() {
        let footnote_index = $(this).attr('id').substr(5)
        let footnoteNum = footnote_index.split('_')[4]
        let verse = $(this).parent().parent().html()
        let verseNum = $(this).parent().parent().prev().children().text()
        $.get('/fnt', {lang: 'ch', fntIdx: footnote_index}).done(function(data) {
            showFootnoteModal(verseNum, footnoteNum, verse, data)
        })
    })
...
var showFootnoteModal = function(titlePrefix, fntNumber, verse, fnt) {
    $('#fntModalTitle').text(titlePrefix + "#" + fntNumber)
    $('#fntModalVerse').html(verse)
    $('#fntModalContent').text(fnt)
    $('#fntModal').modal('show')
}
